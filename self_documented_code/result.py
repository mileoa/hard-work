# Сами по себе комментарии могут вводить в заблуждение, так как при изменении кода их могут просто не поправить.
# Таже комментарии могут быть не оторажающими действительность изначально. Но, вещь которая скорее всего не будет меняться
# и которую стоит попытаться задокументировать достоверно - место класса в системе. Для чего он используется,
# может быть даже указание в каких случаях его не стоит использовать. Например, не стоит использовать класс HTTP в проде, используйте HTTPS.
# Если следовать принципу единой ответственности, то класс действительно будет отвечать только за что-то одно в нашей системе. Таким образом
# наш комментарий не устареет. Также такой комментарий добавит понимания системы в целом, как связаны классы друг с другом - какой класс где используется.
# Ведь какие-то паттерны очевидные для одного человека могут быть неизвестны или неочевидны для другого. К тому же в рамках проекта общепринятые паттерны
# могут иметь модификацию, которая будет неожидана для другого программиста и неявна с первого взгляда на код. Такие комментарии могут помочь понять дизайн
# программы при её первом или повторном просмотре. иногда исключить необходимость чтения самого кода. Иногда даже исключат необходимость чтения реализации
# классов для такого понимания.
